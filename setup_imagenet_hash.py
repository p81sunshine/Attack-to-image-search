...
def prime_function(image_set, image):
    ...
    for i in range(image_set.shape[0]):
        a.append((imagehash.phash(
            gen_image(image_set[i])) - imagehash.phash(gen_image(image))) / 64)
    a = np.asarray(a)
    a = a.astype('float32')
...
def pfunction(arr, arr1):
    ...
    for i in range(arr.shape[0]):
        ...
        a.append(max(
            0, 1 - ((imagehash.phash(gen_image(arr[i])) - imagehash.phash(gen_image(arr1))) / 8)))
...
def pfunction_log(arr, arr1):
    ...
    for i in range(arr.shape[0]):
        a.append(max(
            0, -math.log((imagehash.phash(gen_image(arr[i])) - imagehash.phash(gen_image(arr1))) + 1e-30)))
...
def pfunction_square(arr, arr1):
    ...
    for i in range(arr.shape[0]):
        a.append(
            (max(0, 1 - ((imagehash.phash(gen_image(arr[i])) - imagehash.phash(gen_image(arr1))) / 8)))**2)
...
def pfunction_tanh(arr, arr1):
    a = []
    for i in range(arr.shape[0]):
        a.append(max(0, np.tanh(1 - ((imagehash.phash(gen_image(arr[i]), global_bits, global_factor) - imagehash.phash(
            gen_image(arr1), global_bits, global_factor)) / global_threshold))))
...
def pfunction_tanh2(arr, arr1):
    ...
    for i in range(arr.shape[0]):
        ...
        a.append(max(0, np.tanh(1 - sum(1 for i, j in zip(robusthash.blockhash(newimage),
                 robusthash.blockhash(timage)) if i != j) / global_threshold)))
    a = np.asarray(a)
    a = a.astype('float32')
    return a
...
def pfunction_sigmoid(arr, arr1):
    ...
    differences = []
    for i in range(arr.shape[0]):
        a.append(max(0, sigmoid(
            1 - ((imagehash.phash(gen_image(arr[i])) - imagehash.phash(gen_image(arr1))) / 8)) - 0.5))
        differences.append(imagehash.phash(
            gen_image(arr[i])) - imagehash.phash(gen_image(arr1)))
    print('how is it possible ', differences)
    a = np.asarray(a)
    a = a.astype('float32')
    return a
