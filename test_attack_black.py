...
import warnings
import os
# 0：显示所有日志（默认），1：显示WARNING和以上的日志，2：只显示ERROR日志，3：不显示日志
...
def main(args):
    ...
    with tf.Session(config=config) as sess:
        ...
        data, model = ImageNet(args["start_idx"], args["numimg"], args["path"]), ImageNet_HashModel(
            args['hash'], args['bits'], args['factor'])
        ...
        all_inputs, all_targets, all_labels, all_true_ids, all_gray_inputs = generate_data(data, samples=len(data.test_data),
        ...
        for i in range(args['start_idx'], all_true_ids.size):
            ...
            timestart = time.time()
            adv, const, L3, adv_current, first_iteration = attack.attack_batch(
                gray_inputs, inputs, i, data.file_list[i])
            timeend = time.time()
            ...
            l2_distortion_current = np.sum(
                (adv_current - gray_inputs) ** 2) ** 0.5
            print('l2_distortion between inputs and adv_current ',
                  l2_distortion_current)
            if len(inputs.shape) == 4:
                a, b, c = gray_inputs[0].shape
                print("normalized a,b,c", a, b, c)
                l2_distortion_normalized = l2_distortion_direct / (a*b*c)**0.5
                l2_distortion_current_normalized = l2_distortion_current / \
                    (a*b*c)**0.5
            ...
            distance_t = lpips(image0_ph, image1_ph,
                               model='net-lin', net='alex')

            stacked_gray_inputs = np.asarray(
                np.dstack((stacked_gray_inputs, stacked_gray_inputs, stacked_gray_inputs)))
            stacked_adv = np.asarray(
                np.dstack((stacked_adv, stacked_adv, stacked_adv)))
            stacked_adv_current = np.asarray(
                np.dstack((stacked_adv_current, stacked_adv_current, stacked_adv_current)))
            if len(stacked_gray_inputs.shape) == 3:
                stacked_gray_inputs = stacked_gray_inputs.reshape(
                    (1,) + stacked_gray_inputs.shape)
                stacked_adv = stacked_adv.reshape((1,) + stacked_adv.shape)
                stacked_adv_current = stacked_adv_current.reshape(
                    (1,) + stacked_adv_current.shape)

            print(stacked_adv.shape)
            print(stacked_adv_current.shape)
            # perceptual distances
            with tf.Session(config=config) as session:
                distance1 = session.run(distance_t, feed_dict={image0_ph: (
                    stacked_gray_inputs+0.5), image1_ph: (stacked_adv+0.5)})
                distance2 = session.run(distance_t, feed_dict={image0_ph: (
                    stacked_gray_inputs + 0.5), image1_ph: (stacked_adv_current+0.5)})

            distance1_normalized = distance1[0] / (a*b*c)**0.5
            distance2_normalized = distance2[0] / (a*b*c)**0.5
            success = False
            print("perceptual metrics distance between adv and img",
                  distance1[0])
            print(
                "perceptual metrics distance between adv_current and img", distance2[0])
            print("normalized perceptual metrics distance between adv and img",
                  distance1_normalized)
            print("normalized perceptual metrics distance between adv_current and img",
                  distance2_normalized)

            inputs_img = gen_image(gray_inputs)
            adv_img = gen_image(adv)

            if args["htype"] == "phash":
                hash_differences = imagehash.phash(
                    inputs_img, args['bits'], args['factor']) - imagehash.phash(adv_img, args['bits'], args['factor'])
                ...
                if inputs_img.mode == '1' or inputs_img.mode == 'L' or inputs_img.mode == 'P':
                    ...
                    hash_differences = sum(1 for i, j in zip(robusthash.blockhash(
                        im_original), robusthash.blockhash(im_adver)) if i != j)
            ...
            if L3 == True:
                ...
                if success:
                    ...
                    suffix = "id{}_differ{}_{}_l2{:.2f}_pdistance{}".format(
                        all_true_ids[i], hash_differences, success, l2_distortion_direct, distance1[0])
                   ...
                   # show(adv, "{}/{}/{}_adversarial_{}.png".format(args['save'], args['dataset'], img_no, suffix))
                    ...
                    print("[STATS][L1] total = {}, id = {}, time = {:.3f}, success = {}, const = {:.6f}, hash_avg={:.5f}, distortion = {:.5f}, success_rate = {:.3f}, l2_avg={:.5f}, p_avg={}, iteration_avg={}"
                          .format(img_no, all_true_ids[i], timeend - timestart, success, const, 0 if total_success == 0 else hash_total / total_success, l2_distortion_direct, total_success / float(img_no), 0 if total_success == 0 else l2_total / total_success, 0 if total_success == 0 else pdistance1_total / total_success, 0 if total_success == 0 else total_success_iterations / total_success))
                    sys.stdout.flush()
            else:
                ...
                print("unsuccessful")
                if args["htype"] == "phash":
                    hash_differences_current = imagehash.phash(
                        inputs_img, args['bits'], args['factor']) - imagehash.phash(adv_current_img, args['bits'], args['factor'])
                    print('perceptual hash difference is ',
                          hash_differences_current)
                elif args["htype"] == "blockhash":
                    if inputs_img.mode == '1' or inputs_img.mode == 'L' or inputs_img.mode == 'P':
                        im_original = inputs_img.convert('RGB')
                        im_adver_current = adv_current_img.convert('RGB')
                    hash_differences_current = sum(1 for i, j in zip(robusthash.blockhash(
                        im_original), robusthash.blockhash(im_adver_current)) if i != j)
                ...
                # for name saving purposes, 2nd calculation done
                suffix_current = "l2_{:.2f}_pdist{}_diff{}_success={}_time{}".format(
...
#                show(adv_current, "{}/{}/id{}_adv_current_{}.png".format( args['save'], args['dataset'], i, suffix_current))
            ...
            print("overall average hash ", hash_total2 /
                  (img_no - args["start_idx"]))
            print("overall average l2 ", l2_total2 /
                  (img_no-args["start_idx"]))
            print("overal average perceptual distance ",
                  pdistance_total / (img_no-args["start_idx"]))
            print("overal average iterations ",
                  total_iterations / (img_no-args["start_idx"]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dataset", choices=[
                        "mnist", "cifar10", "imagenet", "maladv", "face", "random"], default="imagenet")
    parser.add_argument("-s", "--save", default="./saved_results")
    parser.add_argument(
        "-a", "--attack", choices=["white", "black"], default="black")
    parser.add_argument("-n", "--numimg", type=int, default=0,
                        help="number of test images to attack")
    parser.add_argument("-m", "--maxiter", type=int,
                        default=0, help="set 0 to use default value")
    parser.add_argument("-p", "--print_every", type=int, default=100,
                        help="print objs every PRINT_EVERY iterations")
    ...
    parser.add_argument("-r", "--reset_adam", action='store_true',
                        help="reset adam after an initial solution is found")
    parser.add_argument("--use_resize", action='store_true',
                        help="resize image (only works on imagenet!)")
    parser.add_argument("--adam_beta1", type=float, default=0.9)
    parser.add_argument("--adam_beta2", type=float, default=0.999)
    parser.add_argument("--seed", type=int, default=1216)
    parser.add_argument("--solver", choices=["adam", "adam2", "adam2_newton",
                        "newton", "adam_newton", "fake_zero"], default="adam")
    parser.add_argument("--save_ckpts", default="",
                        help="path to save checkpoint file")
    parser.add_argument("--load_ckpt", default="",
                        help="path to numpy checkpoint file")
    parser.add_argument("--start_iter", default=0, type=int,
                        help="iteration number for start, useful when loading a checkpoint")
    parser.add_argument("--init_size", default=32, type=int,
                        help="starting with this size when --use_resize")
    parser.add_argument("--uniform", action='store_true',
                        help="disable importance sampling")
    ...
    parser.add_argument(
        "--start_idx", "--start image index", type=int, default=0)
    parser.add_argument(
        "--dct", "--if using dct compression", action='store_true')
    # parser.add_argument("--num_rand_vec", "--random number of vectors like batch", type=int, default=1)
    parser.add_argument("--lr", "--learning rate", type=float, default=0.1)
    parser.add_argument(
        "--transform", "--basic transormation", default="centrol_crop")
    parser.add_argument('--dist_metrics', "--distance metrics to use",
                        choices=["l2dist", "pdist"], default="l2dist")
    parser.add_argument("--bits", "--hash_string_length", type=int, default=8)
    parser.add_argument("--factor", "--hash_string_factor",
                        type=int, default=4)
    parser.add_argument("--maximize", "--if_plus_or_minus",
                        choices=["plus", "minus"], default="minus")
    parser.add_argument(
        "-ht", "--htype", choices=["phash", "blockhash"], default="phash")
    # Image database path
