    ...
    def __init__(self, sess, model, batch_size=1, confidence=CONFIDENCE,
        ...
        if use_tanh:
            ...
            self.timg_grgb = tf.reshape(
                self.timg_grgb, (1, image_height, image_width, num_channels))
...
#            self.modifier_down = 0- img.reshape(-1)
        ...
        for outer_step in range(binary_search_step):
            ...
            self.sess.run(
                self.setup, {self.assign_timg: img, self.assign_const: CONST})
            ...
            if not self.load_checkpoint:
                ...
                rg = np.concatenate((best_modifier, best_modifier), axis=-1)
                rgb = np.concatenate((rg, best_modifier), axis=-1)
                print("loading pretrained greyscale modifier",
                      self.load_checkpoint)
                self.real_modifier = rgb.reshape(
                    (1,) + self.small_single_shape)
                # if checkpoint is incorrect reshape will fail
            ...
            for iteration in range(self.start_iter, self.MAX_ITERATIONS):
                ...
                if iteration % (self.print_every) == 0:
                    print("[STATS][L2] iter = {}, cost = {}, time = {:.3f}, size = {}, loss = {:.5g}, loss1 = {:.5g}, loss2 = {:.5g}".format(
                ...
                if l2 < o_bestl2 and score <= 0:  # and score != 1:
                    ...
                    if o_bestl2 == 1e10:
                        ...
                        print("[STATS][L3](First valid attack found!) iter = {}, cost = {}, time = {:.3f}, size = {}, loss = {:.5g}, loss1 = {:.5g}, loss2 = {:.5g}".format(
                            iteration, eval_costs, train_timer, self.real_modifier.shape, l, loss1, loss2, self.LEARNING_RATE))
                        sys.stdout.flush()
                        L3_count += 1
                        L3 = True
...
#                    o_bestscore = np.argmax(score)
                ...
                if (self.ABORT_EARLY and iteration % self.early_stop_iters == 0) or iteration >= 1000:
                    if l > prev*.9999:
                        print(
                            "Early stopping because there is no improvement or the iterations are too large")
                        break
                    prev = l
                train_timer += time.time() - attack_begin_time
